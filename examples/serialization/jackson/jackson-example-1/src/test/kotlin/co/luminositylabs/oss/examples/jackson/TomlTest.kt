...
class TomlTest {
    ...
    fun `Test data class to toml serialization`() {
        ...
        val appUsers =
            mutableListOf(
                ...
                AppUser(
                    ...
                    emailAddress = "appuser3@everywhere.us",
                    securityPermissions = permissions.subList(0, permissions.size).toSet(),
                    securityRoles = roles.subList(0, roles.size).toSet(),
                    securityRealms = setOf(realms[1]),
                ),
            )
        val tomlAppUsers =
            appUsers
        ...
        val objectAppUsers =
            tomlAppUsers
                .asSequence()
                .map { mapper.readValue(it, AppUser::class.java) }
                .onEach { logger.debug { it } }
                .toList()
        assertEquals(appUsers, objectAppUsers)
    }
}
